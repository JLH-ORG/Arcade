...
namespace Arcade {
	...
	Alien::Alien(glm::vec3 position, glm::vec2 size, float speed, bool canFire, JLHE::EntitySystem& entitySystem, std::vector<Bullet*>& bullets)
		: JLHE::Entity(position, size, 0), m_Speed(speed), m_IsHit(false), m_Countdown(0.0f), m_CanFire(canFire), m_OccupiedEntitySystem(entitySystem), m_Bullets(bullets), m_FireCooldown(0.8f) {}
	...
	void Alien::Fire() {
		if (m_FireCooldown <= 0 && rand() % 1000 < 1) {
			Bullet* b = new Bullet({ m_Position.x, m_Position.y, 0 }, { 0.07f, 0.07f }, -0.5f);
